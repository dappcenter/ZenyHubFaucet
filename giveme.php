...
//2.IPアドレスチェック(time)
//3.アドレスチェック(time)
//4.ZenyHub apiへ送金要求し、結果をjsonでクライアントへ返す
...
//ここでTwitterIDと数値振り分ける(数値優先の為TwitterIDが数値のみの奇特な人はuserid入れてね)

//intならそのまま、その他はAPI使って取る https://open.bitzeny.link/api/v1/twittertoaccount?twitterid=maa123_LQとか

if(!is_numeric($account_id)){
	$api_userid_res=file_get_contents("https://open.bitzeny.link/api/v1/twittertoaccount?twitterid=".urlencode($account_id));
	$api_userid_res=json_decode($api_userid_res,true);
	if($api_userid_res['result']){
		$account_id=$api_userid_res['userid'];
	}
}
...
if($prev_time==='ok'&&$prev_timei==='ok'){
	...
	$re = zf_funcs::send_to($api_key, $hub_userid, $account_id, $reward_zny, $ip_address);
	if($re!==false){
		...
		$res_data=[
		"result"=>false,
		"message"=>'エラーが発生しました。アカウントIDが存在しないかZenyHubAPIとの通信に失敗しました',//単位はZNY
		"r_code"=>1
		];
		header("Content-Type: application/json; charset=utf-8");
		echo json_encode($res_data);
